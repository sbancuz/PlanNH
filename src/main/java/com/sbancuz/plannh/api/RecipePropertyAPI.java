...
public final class RecipePropertyAPI {
    ...
    public static final RecipeResource<ItemStack> ITEM=RecipeResource.<ItemStack>builder("item","Item",null).serialize((obj,stack)->{obj.addProperty("id",Item.itemRegistry.getNameForObject(stack.getItem()));obj.addProperty("size",stack.stackSize);obj.addProperty("meta",stack.getItemDamage());if(stack.hasTagCompound()){obj.addProperty("nbt",stack.getTagCompound().toString());}}).deserialize(obj->{final String id=obj.get("id").getAsString();final int size=obj.get("size").getAsInt();final int meta=obj.get("meta").getAsInt();final Object rawItem=Item.itemRegistry.getObject(id);if(!(rawItem instanceof
    final Item item))return null;
    final ItemStack stack = new ItemStack(item, size, meta);if(obj.has("nbt"))
    {
        try {
    ...
    }return stack;}).displayFormatter(ItemStack::getDisplayName).amountExtractor(stack->stack.stackSize).connectionChecker(ItemStack::isItemEqual).build();
